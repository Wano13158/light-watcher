...
// GPIO кнопки BOOT відрізняється на різних ESP32.
// ESP32-C3: GPIO9, класичний ESP32/DevKit: GPIO0.
// Не можна безумовно чіпати GPIO9 на класичному ESP32, бо це лінія SPI flash;
// pinMode(9, INPUT_PULLUP) може ламати завантаження і викликати reset spam/WDT.
#if CONFIG_IDF_TARGET_ESP32C3
  #define configButtonPin 9
#else
  #define configButtonPin 0
#endif
...
void setup() {
  ...
  // Налаштування запасної кнопки скидання конфігураціїї (BOOT: GPIO9 на ESP32-C3, GPIO0 на класичному ESP32)
  ...
  // Якщо при запуску затиснута кнопка BOOT - запускаємо WEB Сервер для кофігурації
  if (digitalRead(configButtonPin) == LOW) {
    Serial.println("Кнопка BOOT (GPIO " + String(configButtonPin) + ") затиснута. Запуск конфігурації...");
    launchWebServer();
  }
